...
extern "system" {
    ...
    fn GetProcessAffinityMask(process_handle: *mut c_void, cur_mask: *mut usize, system_mask: *mut usize) -> u32;
...
pub fn set_process_affinity<B: AsRef<[usize]>>(core_ids: B) -> Result<()> {
    ...
    if let Err(last_error) = set_process_affinity_mask(cur_proc, wanted_mask) {
        return Err(From::from(format!("SetProcessAffinityMask failed with error 0x{:x}", last_error)));
    }
...
pub fn get_process_affinity() -> Result<Vec<usize>> {
    ...
    match get_process_affinity_mask(cur_proc) {
        ...
        Err(last_error) => {
            return Err(From::from(format!("GetProcessAffinityMask failed with error 0x{:x}", last_error)));
        }
    };
...
pub fn set_thread_affinity(core_ids: &[usize]) -> Result<()> {
    ...
    if let Err(last_error) = set_thread_affinity_mask(cur_thread, wanted_mask) {
        return Err(From::from(format!("SetThreadAffinityMask failed with error 0x{:x}", last_error)));
    }
        ...
        match set_thread_affinity_mask(cur_thread, cur_mask) {
            Ok(mask) => {
                ...
                if let Err(e) = set_thread_affinity_mask(cur_thread, mask) {
                    return Err(From::from(format!("SetThreadAffinityMask failed with error 0x{:x}", e)));
                }
